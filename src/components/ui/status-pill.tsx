...
const statusStyles: Record<string, string> = {
  ...
  rejected:
    "bg-destructive/10 text-destructive hover:text-destructive-foreground border border-destructive/20",
...
export function StatusPill({ status }: { status: string }) {
  return (
    <span
      className={cn(
        "inline-flex rounded-full px-3 py-1 text-[0.75rem] font-semibold",
        statusStyles[status] ?? "bg-muted text-muted-foreground border border-border",
      )}
    >
