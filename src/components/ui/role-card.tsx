    ...
    <Card className="border border-border">
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <span className={cn(
            ...
            remaining > 0 ? "bg-emerald-500/10 text-emerald-900" : "bg-muted text-muted-foreground",
          )}>
      ...
      <CardContent className="space-y-3 pt-0">
        <div className="grid gap-2 text-sm text-muted-foreground">
          <p><span className="font-semibold text-foreground">Responsibilities:</span> {role.responsibilities}</p>
          <p><span className="font-semibold text-foreground">Requirements:</span> {role.requirements}</p>
          <p><span className="font-semibold text-foreground">Skills:</span> {role.skills.join(", ")}</p>
        </div>
        <button
          type="button"
