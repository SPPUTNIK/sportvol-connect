...
export async function getApplications(): Promise<Application[]> {
  ...
  const { data, error } = await supabase
    .from("applications")
    .select(
      "id, status, applied_at, notes, event_id, role_id, event:events(title), role:event_roles(name)",
    )
...
export async function getTraining(): Promise<Training[]> {
  ...
  const [{ data: modules, error: moduleError }, { data: progress, error: progressError }] =
    await Promise.all([
      supabase.from("training_modules").select("id, title, description, resources").order("title", { ascending: true }),
      userId
        ? supabase
...
export async function applyForRole(
  ...
  const notes = [`Availability: ${availability}`, `Experience: ${experience}`, `Motivation: ${motivation}`]
    .filter(Boolean)
    .join("\n\n");
