...
alter table events enable row level security;
create policy events_publish_select on events for select using (
...
alter table event_roles enable row level security;
create policy event_roles_select on event_roles for select using (
  exists(select 1 from events where events.id = event_id and (events.status = 'published' or is_admin()))
);
create policy event_roles_admin on event_roles for insert using (is_admin()) with check (is_admin());
create policy event_roles_admin_update on event_roles for update using (is_admin()) with check (is_admin());
create policy event_roles_admin_delete on event_roles for delete using (is_admin());

alter table applications enable row level security;
create policy applications_self_select on applications for select using (
  profile_id = auth.uid() or is_admin()
);
create policy applications_self_insert on applications for insert with check (
  profile_id = auth.uid()
);
create policy applications_self_update on applications for update using (
  profile_id = auth.uid() or is_admin()
) with check (
  profile_id = auth.uid() or is_admin()
);
...
alter table training_modules enable row level security;
create policy training_modules_select on training_modules for select using (true);
create policy training_modules_admin on training_modules for insert using (is_admin()) with check (is_admin());
create policy training_modules_admin_update on training_modules for update using (is_admin()) with check (is_admin());
create policy training_modules_admin_delete on training_modules for delete using (is_admin());

alter table training_progress enable row level security;
create policy training_progress_self_select on training_progress for select using (
  profile_id = auth.uid() or is_admin()
);
create policy training_progress_self_insert on training_progress for insert with check (
  profile_id = auth.uid()
);
create policy training_progress_self_update on training_progress for update using (
  profile_id = auth.uid() or is_admin()
) with check (
  profile_id = auth.uid() or is_admin()
);
...
alter table event_shifts enable row level security;
create policy event_shifts_select on event_shifts for select using (
  exists(select 1 from events where events.id = event_id and (events.status = 'published' or is_admin()))
);
create policy event_shifts_admin on event_shifts for insert using (is_admin()) with check (is_admin());
create policy event_shifts_admin_update on event_shifts for update using (is_admin()) with check (is_admin());
create policy event_shifts_admin_delete on event_shifts for delete using (is_admin());

alter table shift_assignments enable row level security;
create policy shift_assignments_self_select on shift_assignments for select using (
  profile_id = auth.uid() or is_admin()
);
create policy shift_assignments_self_insert on shift_assignments for insert with check (
  profile_id = auth.uid()
);
create policy shift_assignments_self_update on shift_assignments for update using (
  profile_id = auth.uid() or is_admin()
) with check (
  profile_id = auth.uid() or is_admin()
);
...
alter table attendance_records enable row level security;
create policy attendance_records_self_select on attendance_records for select using (
  profile_id = auth.uid() or is_admin()
);
create policy attendance_records_self_insert on attendance_records for insert with check (
  profile_id = auth.uid()
);
create policy attendance_records_self_update on attendance_records for update using (
  profile_id = auth.uid() or is_admin()
) with check (
  profile_id = auth.uid() or is_admin()
);
...
alter table certificates enable row level security;
create policy certificates_self_select on certificates for select using (
  profile_id = auth.uid() or is_admin()
);
create policy certificates_self_insert on certificates for insert with check (
  profile_id = auth.uid()
);
create policy certificates_self_update on certificates for update using (
  profile_id = auth.uid() or is_admin()
) with check (
  profile_id = auth.uid() or is_admin()
);
...
create policy notifications_self_insert on notifications for insert with check (
  profile_id = auth.uid()
);
...
alter table volunteer_hours enable row level security;
create policy volunteer_hours_self_select on volunteer_hours for select using (
  profile_id = auth.uid() or is_admin()
);
create policy volunteer_hours_self_insert on volunteer_hours for insert with check (
  profile_id = auth.uid()
);
create policy volunteer_hours_self_update on volunteer_hours for update using (
  profile_id = auth.uid() or is_admin()
) with check (
  profile_id = auth.uid() or is_admin()
);
